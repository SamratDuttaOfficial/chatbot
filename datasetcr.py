...
def getWhatsAppData():
    ...
    for i in range (0,100):
        ...
        listsent_i=[]
        listsent_i.append(cleanMessage(listsent[i]))
        listreceived_i=[]
        listreceived_i.append(cleanMessage(listreceived[i]))
        responseDictionary.update(question_patterns = listsent_i, responses = listreceived_i)
...
def cleanMessage(message):
    ...
    cleanedMessage = re.sub(' +',' ', cleanedMessage)
    cleanedMessage = cleanedMessage.encode('ascii', 'ignore').decode('ascii')
